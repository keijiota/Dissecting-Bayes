...
for condition = 1:2
    ...
    for i = 1:9
        errorbar(mean(x(:,i)), mean([y1(:,i)+y2(:,i)]), std(x(:,i)), 'horizontal', 'k', 'linewidth', 1, 'MarkerSize', 0.1, 'CapSize',0); hold on 
        errorbar(mean(x(:,i)), mean([y1(:,i)+y2(:,i)]), [std(y1(:,i))+std(y2(:,i))]/2, 'k', 'linewidth', 1, 'MarkerSize', 0.1, 'CapSize',0); hold on        
