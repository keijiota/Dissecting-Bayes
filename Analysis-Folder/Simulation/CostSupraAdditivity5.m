...
width2a = Sigma(1)*1.01;  % adjusted by a stiar-case
T3a(1) = xc-width2a; T3a(2) = xc+width2a; 
T3a(3) = yc+height/2; T3a(4) = yc-height/2; % top bottom
Target3a = []; Target3a = xx >= T3a(1) & xx < T3a(2) & yy >= T3a(4) & yy <= T3a(3);
hitT3a = samples(:,1) >= T3a(1) & samples(:,1) < T3a(2) & samples(:,2) >= T3a(4) & samples(:,2) <= T3a(3);
probT3a = sum(hitT3a) / nrep;
[probT3a width2 width2a width2a/width2]
...
rectangle('Position',[T3a(1) T3a(4) width2a*2 height]);
plot(samples(:,1),samples(:,2),'k.', 'MarkerSize',11);
plot(mux, muy, 'gs', 'MarkerSize',6); 
axis('equal');
xlim([min(x) max(x)]); ylim([min(y) max(y)]);
