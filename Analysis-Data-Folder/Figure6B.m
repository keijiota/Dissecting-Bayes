...
probaDouble   = sum(hitT1 + hitT2) / nrep ;
superadd = @(p) 1*p + 0.063;
subjective_probaDouble = superadd(probaDouble);
[probaDouble, probT1, probT2, subjective_probaDouble] % P(L)=P(R)=0.2673

hitT3 = samples(:,1) >= T3(1) & samples(:,1) < T3(2) & samples(:,2) >= T3(4) & samples(:,2) <= T3(3);
probaSingle = sum(hitT3) / nrep ;
logodds = @(p) log(p./(1-p));
pm = [0.88 0.76];
subjective_probaSingle = pm(1) * logodds(probaSingle) + (1-pm(1)) * logodds(pm(2));
subjective_probaSingle = exp(subjective_probaSingle) ./ (1 + exp(subjective_probaSingle)) ;
[probaSingle subjective_probaSingle]  % P(S)=0.5346
...
%-------Figure 6B the single target plus the adjusted size------------
figure;
...
%-------Figure 6B supra additivity------------
figure;  hold on
x = 0:0.01:1;
y = superadd(x);
plot(probaDouble,subjective_probaDouble, 'ko', 'MarkerFaceColor','w', 'MarkerSize',7) ; hold on
...
y = exp(y) ./ (1 + exp(y)) ;
plot(probaSingle,subjective_probaSingle, 'ko', 'MarkerFaceColor','w', 'MarkerSize',7) ; hold on
plot(0:0.1:1.2,0:0.1:1.2,'k--');
plot(x, y, 'k-');
axis('square');
xlim([0 1.05]); ylim([0 1.05]);
