...
% the simulation of the expected gain using a emprical sample 
% 1) under the hypothethis of no probability distortion 
% 2) under the hypothethis of linear log-odds transformation
issimulation = 0;
if issimulation == 1
    EG = [];
    for ix = -5:0.05:20
...
% comuputed by the simulation above
optx = 13.15; % the optimal aim point under the hypothethis of no probability distortion 
optlx = 13.30; % the optimal aim point under the hypothethis of linear log-odds transformation
EG = [];
for ix = [optx optlx]
    samples_s = [samples(:,1)+ix samples(:,2)]; 
    dis2penalty = sum((samples_s - penalty_center).^2, 2).^(1/2); dis2reward = sum((samples_s - reward_center).^2, 2).^(1/2);
